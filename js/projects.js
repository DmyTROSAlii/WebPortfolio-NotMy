...
const changeMode = document.getElementById('change-visible');
const activeFilters = {};
...
const handleFormChange = (event) => {
	...
	if (!projectsContainer.contains(document.querySelector('.project-card'))) {
		projectsContainer.innerHTML = '<div class="filter-output" style="text-align: center; font-size: 22px;">There are not items that satisfy the filter</div>';
...
changeMode.addEventListener('click', () => {
	document.querySelectorAll('.project-card').forEach(card => {card.classList.toggle('horisontal-look')});
});
